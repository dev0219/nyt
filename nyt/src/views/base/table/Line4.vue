<template>
  <CCard>
    <CCardHeader  class="pointer" @click="line4()">
      <slot name="header">
...
export default {
  ...
  methods: {
  BrowStatus(){
    if(localStorage.getItem('BR_D_Line4') == '' || localStorage.getItem('BR_D_Line4') == null){
      this.browstatus = true
    }else if(localStorage.getItem('BR_D_Line4') == 'BR_Sai5_04'){
  ...
  async appData(){
    ...
    let apiCounts_String = localStorage.getItem('api_counts_line4');
    let apiCounts = 0;
    if(apiCounts != null) {
      apiCounts = parseInt(apiCounts_String);
    }
    let dd = localStorage.getItem('initial_progress_line4');
    if(dd == null || apiCounts%60==0) {
      apiCounts = 0;
      dd= (geteffective/this.DataList4.length).toFixed(2);
      localStorage.setItem('initial_progress_line4', Number((geteffective/this.DataList4.length).toFixed(2))); 
    }
    localStorage.setItem('api_counts_line4', apiCounts++);
    this.effective = Number(dd);
    localStorage.setItem('progress_linedetail_line4', this.effective);
