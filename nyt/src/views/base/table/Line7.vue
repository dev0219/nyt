<template>
  <CCard>
    <CCardHeader  class="pointer" @click="line7()">
      <slot name="header">
...
export default {
  ...
  methods: {
  BrowStatus(){
    if(localStorage.getItem('BR_D_Line7') == '' || localStorage.getItem('BR_D_Line7') == null){
      this.browstatus = true
    }else if(localStorage.getItem('BR_D_Line7') == 'BR_Sai5_07'){
  ...
  async appData(){
    ...
    let apiCounts_String = localStorage.getItem('api_counts_line7');
    let apiCounts = 0;
    if(apiCounts != null) {
      apiCounts = parseInt(apiCounts_String);
    }
    let dd = localStorage.getItem('initial_progress_line7');
    if(dd == null || apiCounts%60==0) {
      apiCounts = 0;
      dd= (geteffective/this.DataList7.length).toFixed(2);
      localStorage.setItem('initial_progress_line7', Number((geteffective/this.DataList7.length).toFixed(2))); 
    }
    localStorage.setItem('api_counts_line7', apiCounts++);
    this.effective = Number(dd);
    localStorage.setItem('progress_linedetail_line7', this.effective);
