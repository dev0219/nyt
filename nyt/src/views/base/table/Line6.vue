<template>
  <CCard>
    <CCardHeader class="pointer" @click="line6()">
      <slot name="header">
...
export default {
  ...
  methods: {
  BrowStatus(){
    if(localStorage.getItem('BR_D_Line6') == '' || localStorage.getItem('BR_D_Line6') == null){
      this.browstatus = true
    }else if(localStorage.getItem('BR_D_Line6') == 'BR_Sai5_06'){
  ...
  async appData(){
    ...
    let apiCounts_String = localStorage.getItem('api_counts_line6');
    let apiCounts = 0;
    if(apiCounts != null) {
      apiCounts = parseInt(apiCounts_String);
    }
    let dd = localStorage.getItem('initial_progress_line6');
    if(dd == null || apiCounts%60==0) {
      apiCounts = 0;
      dd= (geteffective/this.DataList6.length).toFixed(2);
      localStorage.setItem('initial_progress_line6', Number((geteffective/this.DataList6.length).toFixed(2))); 
    }
    localStorage.setItem('api_counts_line6', apiCounts++);
    this.effective = Number(dd);
    localStorage.setItem('progress_linedetail_line6', this.effective);
