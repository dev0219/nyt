<template>
  <CCard>
    <CCardHeader class="pointer" @click="line2()">
      <slot name="header">
...
export default {
  ...
  methods: {
  BrowStatus(){
    if(localStorage.getItem('BR_D_Line2') == '' || localStorage.getItem('BR_D_Line2') == null){
      this.browstatus = true
    }else if(localStorage.getItem('BR_D_Line2') == 'BR_Sai5_02'){
  ...
  async appData(){
    ...
    let apiCounts_String = localStorage.getItem('api_counts_line2');
    let apiCounts = 0;
    if(apiCounts != null) {
      apiCounts = parseInt(apiCounts_String);
    }
    let dd = localStorage.getItem('initial_progress_line2');
    if(dd == null || apiCounts%60==0) {
      apiCounts = 0;
      dd= (geteffective/this.DataList2.length).toFixed(2);
      localStorage.setItem('initial_progress_line2', Number((geteffective/this.DataList2.length).toFixed(2))); 
    }
    localStorage.setItem('api_counts_line2', apiCounts++);
    this.effective = Number(dd);
    localStorage.setItem('progress_linedetail_line2', this.effective);
