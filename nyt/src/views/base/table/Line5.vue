<template>
  <CCard>
    <CCardHeader class="pointer" @click="line5()">
      <slot name="header">
...
export default {
  ...
  methods: {
  BrowStatus(){
    if(localStorage.getItem('BR_D_Line5') == '' || localStorage.getItem('BR_D_Line5') == null){
      this.browstatus = true
    }else if(localStorage.getItem('BR_D_Line5') == 'BR_Sai5_05'){
  ...
  async appData(){
    ...
    let apiCounts_String = localStorage.getItem('api_counts_line5');
    let apiCounts = 0;
    if(apiCounts != null) {
      apiCounts = parseInt(apiCounts_String);
    }
    let dd = localStorage.getItem('initial_progress_line5');
    if(dd == null || apiCounts%60==0) {
      apiCounts = 0;
      dd= (geteffective/this.DataList5.length).toFixed(2);
      localStorage.setItem('initial_progress_line5', Number((geteffective/this.DataList5.length).toFixed(2))); 
    }
    localStorage.setItem('api_counts_line5', apiCounts++);
    this.effective = Number(dd);
    localStorage.setItem('progress_linedetail_line5', this.effective);
