<template>
  <CCard>
    <CCardHeader class="pointer" @click="line8()">
      <slot name="header">
...
export default {
  ...
  methods: {
  BrowStatus(){
    if(localStorage.getItem('BR_D_Line8') == '' || localStorage.getItem('BR_D_Line8') == null){
      this.browstatus = true
    }else if(localStorage.getItem('BR_D_Line8') == 'BR_Sai5_08'){
  ...
  async appData(){
    ...
    let apiCounts_String = localStorage.getItem('api_counts_line8');
    let apiCounts = 0;
    if(apiCounts != null) {
      apiCounts = parseInt(apiCounts_String);
    }
    let dd = localStorage.getItem('initial_progress_line8');
    if(dd == null || apiCounts%60==0) {
      apiCounts = 0;
      dd= (geteffective/this.DataList8.length).toFixed(2);
      localStorage.setItem('initial_progress_line8', Number((geteffective/this.DataList8.length).toFixed(2))); 
    }
    localStorage.setItem('api_counts_line8', apiCounts++);
    this.effective = Number(dd);
    localStorage.setItem('progress_linedetail_line8', this.effective);
