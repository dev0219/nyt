<template>
  <CCard>
    <CCardHeader  class="pointer" @click="line3()">
      <slot name="header">
...
export default {
  ...
  methods: {
  BrowStatus(){
    if(localStorage.getItem('BR_D_Line3') == '' || localStorage.getItem('BR_D_Line3') == null){
      this.browstatus = true
    }else if(localStorage.getItem('BR_D_Line3') == 'BR_Sai5_03'){
  ...
  async appData(){
    ...
    let apiCounts_String = localStorage.getItem('api_counts_line3');
    let apiCounts = 0;
    if(apiCounts != null) {
      apiCounts = parseInt(apiCounts_String);
    }
    let dd = localStorage.getItem('initial_progress_line3');
    if(dd == null || apiCounts%60==0) {
      apiCounts = 0;
      dd= (geteffective/this.DataList3.length).toFixed(2);
      localStorage.setItem('initial_progress_line3', Number((geteffective/this.DataList3.length).toFixed(2))); 
    }
    localStorage.setItem('api_counts_line3', apiCounts++);
    this.effective = Number(dd);
    localStorage.setItem('progress_linedetail_line3', this.effective);
