              ...
              <!-- <CCol sm="6">
                ...
                <div class="effective">
                  ...
                  v-model="eff"
                   ...
                   min="70"
                  />
                   <div class="assign"><b>COLOR:</b><input  id="line7" type="color" v-model="color" @change="colorset()"/></div>
                </div>
              </CCol> -->
              <CCol sm="3">
...
export default {
  ...
  async created(){
      ...
      if(DataList[i].PD_Line == "7"){   
        if(this.DataList7.length < 6){
          ...
          if(DataList[i].EFF >= 90){
            ...
            if(localStorage.getItem('machinecolor7') == '' || localStorage.getItem('machinecolor7') == null){
              DataList[i].NP = "N"
              DataList[i]['_classes'] = "bg-warning"
            }else{
              DataList[i].NP = "N"
              $(function(){
                 var dd =  localStorage.getItem('machinecolor7') 
    ...
    this.effective = Number(localStorage.getItem('progress_linedetail_line7'))
    ...
    this.DataListSel7 = this.DataList7
  },
  methods: {
    colorset(){
      localStorage.setItem('machinecolor7', this.color)
    ...
    SelStatus(){
      if(this.SelMachine == "all"){
        ...
        var  greendataItem=[];
        var  greendata= [];
        for (var i=0 ; i< this.DataList7.length;i++){
          if(this.DataList7[i]['EFF'] >= 90){
            greendataItem = this.DataList7[i]
            greendata.push(greendataItem)
          }
          
        }
        this.DataListSel7 = greendata
      }else if(this.SelMachine == "yellow"){
        var  yellowdataItem=[];
        var  yellowdata= [];
        for (var i=0 ; i< this.DataList7.length;i++){
          if(this.DataList7[i]['EFF'] >= 70 && this.DataList7[i]['EFF'] < 90){
            yellowdataItem = this.DataList7[i]
            yellowdata.push(yellowdataItem)
          }
        } 
        this.DataListSel7 = yellowdata
      }else if(this.SelMachine == "red"){
        var  reddataItem=[];
        var  reddata= [];
        for (var i=0 ; i< this.DataList7.length;i++){
          if(this.DataList7[i]['EFF'] < 70){
            reddataItem = this.DataList7[i]
            reddata.push(reddataItem)
          }          
        }
        this.DataListSel7 = reddata      }
    },
    shuffleArray (array) {
      return array
    },
    getShuffledUsersData () {
      return this.shuffleArray(this.DataListSel7.slice(0))
    }
  }
}
</script>
