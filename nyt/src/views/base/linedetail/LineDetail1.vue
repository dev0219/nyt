              ...
              <!-- <CCol sm="6">
                ...
                <div class="effective">
                  ...
                  v-model="eff"
                   ...
                   min="70"
                  />
                   <div class="assign"><b>COLOR:</b><input  id="line1" type="color" v-model="color" @change="colorset()"/></div>
                </div>
              </CCol> -->
              <CCol sm="3">
...
export default {
  ...
  async created(){
      ...
      if(DataList[i].PD_Line == "1"){   
        if(this.DataList1.length < 6){
          ...
          if(DataList[i].EFF >= 90){
            ...
            if(localStorage.getItem('machinecolor1') == '' || localStorage.getItem('machinecolor1') == null){
              DataList[i].NP = "N"
              DataList[i]['_classes'] = "bg-warning"
            }else{
              DataList[i].NP = "N"
              $(function(){
                 var dd =  localStorage.getItem('machinecolor1') 
    ...
    this.effective = Number(localStorage.getItem('progress_linedetail_line1'))
    ...
    this.DataListSel1 = this.DataList1
  },
  methods: {
    colorset(){
      localStorage.setItem('machinecolor1', this.color)
    ...
    SelStatus(){
      if(this.SelMachine == "all"){
        ...
        for (var i=0 ; i< this.DataList1.length;i++){
          if(this.DataList1[i]['EFF'] >= 90){
            greendataItem = this.DataList1[i]
            greendata.push(greendataItem)
          }          
        }
        this.DataListSel1 = greendata
      }else if(this.SelMachine == "yellow"){
        var  yellowdataItem=[];
        var  yellowdata= [];
        for (var i=0 ; i< this.DataList1.length;i++){
          if(this.DataList1[i]['EFF'] >= 70 && this.DataList1[i]['EFF'] < 90){
            yellowdataItem = this.DataList1[i]
            yellowdata.push(yellowdataItem)
          }
        } 
        this.DataListSel1 = yellowdata
      }else if(this.SelMachine == "red"){
        var  reddataItem=[];
        var  reddata= [];
        for (var i=0 ; i< this.DataList1.length;i++){
          if(this.DataList1[i]['EFF'] < 70){
            reddataItem = this.DataList1[i]
            reddata.push(reddataItem)
          }          
        }
        this.DataListSel1 = reddata      }
    },
    shuffleArray (array) {
      return array
    },
    getShuffledUsersData () {
      return this.shuffleArray(this.DataListSel1.slice(0))
    }
  }
}
</script>
