              ...
              <!-- <CCol sm="6">
                ...
                <div class="effective">
                  ...
                  v-model="eff"
                   ...
                   min="70"
                  />
                   <div class="assign"><b>COLOR:</b><input  id="line8" type="color" v-model="color" @change="colorset()"/></div>
                </div>
              </CCol> -->
              <CCol sm="3">
...
export default {
  ...
  async created(){
      ...
      if(DataList[i].PD_Line == "8"){   
        if(this.DataList8.length < 6){
          ...
          if(DataList[i].EFF >= 90){
            ...
            if(localStorage.getItem('machinecolor8') == '' || localStorage.getItem('machinecolor8') == null){
              DataList[i].NP = "N"
              DataList[i]['_classes'] = "bg-warning"
            }else{
              DataList[i].NP = "N"
              $(function(){
                 var dd =  localStorage.getItem('machinecolor8') 
    ...
    this.effective = Number(localStorage.getItem('progress_linedetail_line8'))
    ...
    this.DataListSel8 = this.DataList8
  },
  methods: {
    colorset(){
      localStorage.setItem('machinecolor8', this.color)
    ...
    SelStatus(){
      if(this.SelMachine == "all"){
        ...
        var  greendataItem=[];
        var  greendata= [];
        for (var i=0 ; i< this.DataList8.length;i++){
          if(this.DataList8[i]['EFF'] >= 90){
            greendataItem = this.DataList8[i]
            greendata.push(greendataItem)
          }
          
        }
        this.DataListSel8 = greendata
      }else if(this.SelMachine == "yellow"){
        var  yellowdataItem=[];
        var  yellowdata= [];
        for (var i=0 ; i< this.DataList8.length;i++){
          if(this.DataList8[i]['EFF'] >= 70 && this.DataList8[i]['EFF'] < 90){
            yellowdataItem = this.DataList8[i]
            yellowdata.push(yellowdataItem)
          }
        } 
        this.DataListSel8 = yellowdata
      }else if(this.SelMachine == "red"){
        var  reddataItem=[];
        var  reddata= [];
        for (var i=0 ; i< this.DataList8.length;i++){
          if(this.DataList8[i]['EFF'] < 70){
            reddataItem = this.DataList8[i]
            reddata.push(reddataItem)
          }          
        }
        this.DataListSel8 = reddata      }
    },
    shuffleArray (array) {
      return array
    },
    getShuffledUsersData () {
      return this.shuffleArray(this.DataListSel8.slice(0))
    }
  }
}
</script>
