              ...
              <!-- <CCol sm="6">
                ...
                <div class="effective">
                  ...
                  v-model="eff"
                   ...
                   min="70"
                  />
                   <div class="assign"><b>COLOR:</b><input  id="line6" type="color" v-model="color" @change="colorset()"/></div>
                </div>
              </CCol> -->
              <CCol sm="3">
...
export default {
  ...
  async created(){
      ...
      if(DataList[i].PD_Line == "6"){   
        if(this.DataList6.length < 6){
          ...
          if(DataList[i].EFF >= 90){
            ...
            if(localStorage.getItem('machinecolor6') == '' || localStorage.getItem('machinecolor6') == null){
              DataList[i].NP = "N"
              DataList[i]['_classes'] = "bg-warning"
            }else{
              DataList[i].NP = "N"
              $(function(){
                 var dd =  localStorage.getItem('machinecolor6') 
    ...
    this.effective = Number(localStorage.getItem('progress_linedetail_line6'))
    ...
    this.DataListSel6 = this.DataList6
  },
  methods: {
    colorset(){
      localStorage.setItem('machinecolor6', this.color)
    ...
    SelStatus(){
      if(this.SelMachine == "all"){
        ...
        var  greendataItem=[];
        var  greendata= [];
        for (var i=0 ; i< this.DataList6.length;i++){
          if(this.DataList6[i]['EFF'] >= 90){
            greendataItem = this.DataList6[i]
            greendata.push(greendataItem)
          }
          
        }
        this.DataListSel6 = greendata
      }else if(this.SelMachine == "yellow"){
        var  yellowdataItem=[];
        var  yellowdata= [];
        for (var i=0 ; i< this.DataList6.length;i++){
          if(this.DataList6[i]['EFF'] >= 70 && this.DataList6[i]['EFF'] < 90){
            yellowdataItem = this.DataList6[i]
            yellowdata.push(yellowdataItem)
          }
        } 
        this.DataListSel6 = yellowdata
      }else if(this.SelMachine == "red"){
        var  reddataItem=[];
        var  reddata= [];
        for (var i=0 ; i< this.DataList6.length;i++){
          if(this.DataList6[i]['EFF'] < 70){
            reddataItem = this.DataList6[i]
            reddata.push(reddataItem)
          }          
        }
        this.DataListSel6 = reddata      }
    },
    shuffleArray (array) {
      return array
    },
    getShuffledUsersData () {
      return this.shuffleArray(this.DataListSel6.slice(0))
    }
  }
}
</script>
