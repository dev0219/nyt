              ...
              <!-- <CCol sm="6">
                ...
                <div class="effective">
                  ...
                  v-model="eff"
                   ...
                   min="70"
                  />
                   <div class="assign"><b>COLOR:</b><input  id="line3" type="color" v-model="color" @change="colorset()"/></div>
                </div>
              </CCol> -->
              <CCol sm="3">
...
export default {
  ...
  async created(){
        ...
        if(this.DataList3.length < 6){
         ...
         DataList[i].Actual =((DataList[i].weight_gram/1000).toFixed(2)).toString();
          ...
          if(DataList[i].EFF >= 90){
            ...
            if(localStorage.getItem('machinecolor3') == '' || localStorage.getItem('machinecolor3') == null){
              DataList[i].NP = "N"
              DataList[i]['_classes'] = "bg-warning"
            }else{
              DataList[i].NP = "N"
              $(function(){
                 var dd =  localStorage.getItem('machinecolor3') 
    ...
    this.effective = Number(localStorage.getItem('progress_linedetail_line3'))
    ...
    this.DataListSel3 = this.DataList3
  },
  methods: {
    colorset(){
      localStorage.setItem('machinecolor3', this.color)
    ...
    SelStatus(){
      if(this.SelMachine == "all"){
        ...
        for (var i=0 ; i< this.DataList3.length;i++){
          if(this.DataList3[i]['EFF'] >= 90){
            greendataItem = this.DataList3[i]
            greendata.push(greendataItem)
          }
          
        }
        this.DataListSel3 = greendata
      }else if(this.SelMachine == "yellow"){
        var  yellowdataItem=[];
        var  yellowdata= [];
        for (var i=0 ; i< this.DataList3.length;i++){
          if(this.DataList3[i]['EFF'] >= 70 && this.DataList3[i]['EFF'] < 90){
            yellowdataItem = this.DataList3[i]
            yellowdata.push(yellowdataItem)
          }
        } 
        this.DataListSel3 = yellowdata
      }else if(this.SelMachine == "red"){
        var  reddataItem=[];
        var  reddata= [];
        for (var i=0 ; i< this.DataList3.length;i++){
          if(this.DataList3[i]['EFF'] < 70){
            reddataItem = this.DataList3[i]
            reddata.push(reddataItem)
          }          
        }
        this.DataListSel3 = reddata      }
    },
    shuffleArray (array) {
      return array
    },
    getShuffledUsersData () {
      return this.shuffleArray(this.DataListSel3.slice(0))
    }
  }
}
</script>
