              ...
              <!-- <CCol sm="6">
                ...
                <div class="effective">
                  ...
                  v-model="eff"
                   ...
                   min="70"
                  />
                   <div class="assign"><b>COLOR:</b><input  id="line4" type="color" v-model="color" @change="colorset()"/></div>
                </div>
              </CCol> -->
              <CCol sm="3">
...
export default {
  ...
  async created(){
      ...
      if(DataList[i].PD_Line == "4"){   
        if(this.DataList4.length < 6){
          ...
          if(DataList[i].EFF >= 90){
            ...
            if(localStorage.getItem('machinecolor4') == '' || localStorage.getItem('machinecolor4') == null){
              DataList[i].NP = "N"
              DataList[i]['_classes'] = "bg-warning"
            }else{
              DataList[i].NP = "N"
              $(function(){
                 var dd =  localStorage.getItem('machinecolor4') 
    ...
    this.effective = Number(localStorage.getItem('progress_linedetail_line4'))
    ...
    this.DataListSel4 = this.DataList4
  },
  methods: {
    colorset(){
      localStorage.setItem('machinecolor4', this.color)
     ...
     SelStatus(){
      if(this.SelMachine == "all"){
        ...
        var  greendataItem=[];
        var  greendata= [];
        for (var i=0 ; i< this.DataList4.length;i++){
          if(this.DataList4[i]['EFF'] >= 90){
            greendataItem = this.DataList4[i]
            greendata.push(greendataItem)
          }
          
        }
        this.DataListSel4 = greendata
      }else if(this.SelMachine == "yellow"){
        var  yellowdataItem=[];
        var  yellowdata= [];
        for (var i=0 ; i< this.DataList4.length;i++){
          if(this.DataList4[i]['EFF'] >= 70 && this.DataList4[i]['EFF'] < 90){
            yellowdataItem = this.DataList4[i]
            yellowdata.push(yellowdataItem)
          }
        } 
        this.DataListSel4 = yellowdata
      }else if(this.SelMachine == "red"){
        var  reddataItem=[];
        var  reddata= [];
        for (var i=0 ; i< this.DataList4.length;i++){
          if(this.DataList4[i]['EFF'] < 70){
            reddataItem = this.DataList4[i]
            reddata.push(reddataItem)
          }          
        }
        this.DataListSel4 = reddata      }
    },
    shuffleArray (array) {
      return array
    },
    getShuffledUsersData () {
      return this.shuffleArray(this.DataListSel4.slice(0))
    }
  }
}
</script>
